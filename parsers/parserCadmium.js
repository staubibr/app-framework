...
export default class ParserCadmium extends Parser { 
	...
	async parse_metadata() {
		...
		tokens.forEach(t => {
			var curr = metadata.get_model(t.id);
	...
	async parse_messages(simulation) {
		...
		var o_frames = await this.parse_file(simulation, this.files.output, this.parse_output_line);
	 ...
	 * @return {Frame[]} an array of frames built from the messages.log
	 */		
	async parse_file(simulation, file, fn) {
		var t = null;
		this.frame = null;
		...
		return Reader.read_by_chunk(file, "\n", (parsed, chunk, progress) => {
			...
			for (var i = 0; i < lines.length; i++) {
				...
				else fn(this.frame, simulation, line);
			}
	 ...
	 */	
	parse_state_line(frame, simulation, line) {
		// examples
		...
		if (model) frame.add_state_message(new MessageState(simulation.get_model(name), [value]));	
	}
	 ...
	 * @param {string[]} data - the split message
	 */	
	parse_output_line(frame, simulation, line) {
		// examples
		...
		var model = simulation.get_model(name);
